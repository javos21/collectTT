...
/**
 * Store staff receive incoming items and may return unresolved holdings.
 *
...
function counterUrl(storeId: string, query: string): string {
  return `/store/${storeId}?${query}&mode=receive`;
}
...
export async function lookupByCodeAction(formData: FormData): Promise<void> {
  ...
  redirect(counterUrl(storeId, `lookup=${encodeURIComponent(found.holdingId)}`));
}
